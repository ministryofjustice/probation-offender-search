...
internal class DurationHelperTest {
  companion object {
    ...
    fun datesAndDurations() = listOf(
      ...
      arguments(
        DateTimeFormatter.ISO_DATE_TIME.format(LocalDateTime.now().minusMinutes(4)),
        Duration.ofMinutes(5),
        false
      ),
      arguments(
        DateTimeFormatter.ISO_DATE_TIME.format(LocalDateTime.now().minusMinutes(6)),
        Duration.ofMinutes(5),
        true
      )
    )
  ...
  internal fun `will determine if a string is a date longer ago than the duration`(
    date: String,
    duration: Duration,
    expected: Boolean
  ) {
