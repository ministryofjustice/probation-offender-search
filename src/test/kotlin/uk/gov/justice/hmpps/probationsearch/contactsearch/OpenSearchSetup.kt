...
class OpenSearchSetup {
    ...
    fun setup() {
        ...
        restClient.performRequest(Request("PUT", "/_index_template/contact-semantic-index-template").apply {
            setJsonEntity(INDEX_TEMPLATE_JSON)
        })
        restClient.performRequest(Request("PUT", "/_index_template/contact-semantic-block-template").apply {
        ...
        restClient.performRequest(Request("PUT", "/_ingest/pipeline/contact-semantic-search-pipeline").apply {
            setJsonEntity(INGEST_PIPELINE_JSON.replace("\${model_id}", modelId))
        })
        restClient.performRequest(Request("PUT", "/_search/pipeline/contact-semantic-search-search-pipeline").apply {
            setJsonEntity(SEARCH_PIPELINE_JSON.replace("\${model_id}", modelId))
        })
        restClient.performRequest(Request("PUT", "/_ingest/pipeline/contact-semantic-block-pipeline").apply {
