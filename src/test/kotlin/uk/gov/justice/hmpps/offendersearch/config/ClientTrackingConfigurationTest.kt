...
import uk.gov.justice.hmpps.offendersearch.utils.findLogAppender

@Import(JwtConfig::class, JwtAuthenticationHelper::class, ClientTrackingInterceptor::class, ClientTrackingConfiguration::class)
...
class ClientTrackingConfigurationTest {
  ...
  fun shouldAddClientIdAndUserNameToInsightTelemetry() {
    val token = jwtAuthHelper.createJwt(username = "bob")
  ...
  fun shouldAddOnlyClientIdIfUsernameNullToInsightTelemetry() {
    ...
    clientTrackingInterceptor.preHandle(req, res, "null")
    val insightTelemetry = ThreadContext.getRequestTelemetryContext().httpRequestTelemetry.properties
    assertThat(insightTelemetry).containsExactlyInAnyOrderEntriesOf(mapOf("clientId" to "new-tech"))
  }
