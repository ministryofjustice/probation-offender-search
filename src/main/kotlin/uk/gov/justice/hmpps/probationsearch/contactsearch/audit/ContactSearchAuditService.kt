...
class ContactSearchAuditService(
  ...
  fun audit(request: ContactSearchRequest, pageable: Pageable) {
    val scope = CoroutineScope(Context.current().asContextElement() + SecurityCoroutineContext())
    auditService?.run {
      scope.launch {
        publishEvent(
    ...
    deliusService.auditActivitySearch(
      ActivitySearchAuditRequest(
        ...
        ActivitySearchAuditRequest.PageRequest(
          pageable.pageNumber,
          pageable.pageSize,
          fieldSorts.mapNotNull { ActivitySearchService.SortType.from(it.fieldName)?.aliases?.first() }.joinToString(),
          fieldSorts.joinToString { it.order().toString() },
        ),
      ),
    )
